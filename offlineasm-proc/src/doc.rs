...
use crate::{asm::Assembler, ast::{Instruction, LabelMapping, Node}};
        ...
        match self {
            ...
            Node::Instruction(instr) => {
                ...
                if let Some(doc) = instr.documentation.as_ref() {
                    ...
                    let index =
                        INSTRUCTION_INDEX.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    let const_name =
                        Ident::new(&format!("INSTRUCTION_{}", index), name.span());
            ...
            Node::Label(label) => {
                ...
                let const_name = Ident::new(&format!("LABEL_{}_{}", label.name(), index), label.name().span());    
                let lit = LitStr::new(&format!("global label '{}'", label.name()), label.name().span());
