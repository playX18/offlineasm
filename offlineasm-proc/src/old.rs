...
impl Parse for ParseAsm {
    fn parse(mut input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        *AUTO_CFG_STORAGE.lock() = conds
        ...
        let node = parser.parse_sequence_inner(&mut input)?;

        Ok(ParseAsm {
            cond_constants: conds,
            toplevel: node,
        })
    }
}
...
pub fn offlineasm_expand(input: TokenStream) -> TokenStream {
    ...
    let mut conds = node.cond_constants.clone();
    let (node, directives) = node.toplevel.directives();
    if directives.contains(&id("force_rloop")) || !is_x86_64() {
        println!("ADD RLOOP");
        AUTO_CFG_STORAGE.lock().insert("rloop".to_string(), true);
        conds.insert(id("rloop"), true);
    }
    let node = node
        .resolve_settings(&conds)
    ...
    if directives.contains(&id("force_rloop")) || !is_x86_64() {
        ...
        let mut rloop = RLoopEmitter::new();

        let code = rloop
            .compile_sequence(if let Node::Seq(ref seq) = node {
                seq
            } else {
                &[]
            })
